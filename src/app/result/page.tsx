...
export default function ResultPage() {
  ...
  if (!result || typeof result !== 'object' || !('shape' in result)) return <div className="max-w-lg mx-auto mt-24 text-center text-lg text-[#a656c2]">Loading result...</div>;
  const r = result as { imageUrl: string, shape: string, shapeScores?: Record<string, number> };
  return (
      ...
      <div className="flex flex-col md:flex-row gap-8 items-center">
        <div className="flex-1 flex flex-col items-center gap-3">
          ...
          <div className="relative w-[240px] h-[240px]">
            <img src={r.imageUrl} alt="User" className="rounded-lg shadow max-w-full h-auto" />
            {r.overlayData && (<canvas width={240} height={240} style={{position:'absolute',top:0,left:0,pointerEvents:'none'}} />)}
          </div>
        </div>
        <div className="flex-1 flex flex-col gap-2">
          <div className="flex flex-row gap-3 items-center md:items-start">
            <img src={shapeMeta[r.shape].icon} alt="icon" width={54} height={54} />
            <div>
              <div className="text-lg tracking-wide font-semibold text-[#a656c2]">Your face shape:</div>
              <div className="text-3xl font-extrabold text-[#453670] mb-1">{r.shape}</div>
            </div>
          </div>
          <div className="text-[#6c6a74] mb-6">{shapeMeta[r.shape].desc}</div>
            ...
            <div className="flex flex-col gap-1 w-full">
              {faceShapes.map(shape=>(
                <div key={shape} className="flex gap-2 items-center">
                  ...
                  <div className="bg-[#c9c8ca]/30 flex-1 rounded-r h-3 relative"><div className="absolute top-0 left-0 h-3 bg-[#a656c2] rounded-r" style={{width:(r.shapeScores?Math.round(r.shapeScores[shape]*100):0)+"%"}}></div></div>
                  <div style={{width:28,textAlign:'right'}}>{r.shapeScores?Math.round(r.shapeScores[shape]*100):0}%</div>
                </div>
              ))}
            </div>
          </div>
